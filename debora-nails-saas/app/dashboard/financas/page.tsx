...
import { Wallet, TrendingUp, TrendingDown, Plus, DollarSign, X, Loader2, ArrowUpRight, ArrowDownRight, Trash2, Calendar, Percent, CircleDollarSign, Edit2 } from 'lucide-react';
...
export default function FinancasPage() {
  ...
  const handleSalvarTransacao = async (e: React.FormEvent) => {
    ...
    const dadosTransacao = {
      ...
      data_pagamento: new Date(`${formData.get('data')}T12:00:00`).toISOString()
    };

    if (transacaoEditando) {
      const { error } = await supabase.from('transacoes').update(dadosTransacao).eq('id', transacaoEditando.id);
      if (error) alert('Erro ao atualizar registro financeiro.');
    } else {
      const { error } = await supabase.from('transacoes').insert([dadosTransacao]);
      if (error) alert('Erro ao registrar transação.');
    }
            ...
            {transacoes.map(t => {
              ...
              return (
                <div key={t.id} className="bg-[#120308]/80 border border-[#DCAE96]/10 p-4 rounded-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:border-[#DCAE96]/30 transition-colors">
                  ...
                  <div className="flex items-center justify-between w-full md:w-auto gap-4 shrink-0 mt-2 md:mt-0 pt-2 md:pt-0 border-t border-[#DCAE96]/10 md:border-none">
                    ...
                    <div className="flex items-center gap-1">
                      <button onClick={() => abrirModalEdicao(t)} className="text-gray-400 hover:text-[#F8D1BE] hover:bg-[#DCAE96]/10 rounded-lg transition-colors p-2 shrink-0" title="Editar Lançamento">
                        <Edit2 size={16} />
                      </button>
                      <button onClick={() => deletarTransacao(t.id)} className="text-gray-500 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors p-2 shrink-0" title="Excluir Lançamento">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/80 backdrop-blur-sm px-4">
          <div className="bg-[#120308] border border-[#DCAE96]/40 rounded-3xl w-full max-w-md overflow-hidden shadow-[0_0_40px_rgba(199,151,125,0.2)] animate-in zoom-in-95 duration-300 max-h-[90dvh] flex flex-col">
            <div className="bg-[#2D0A12] px-6 py-4 flex justify-between items-center border-b border-[#DCAE96]/20 shrink-0">
              <h2 className="text-xl font-serif text-[#F8D1BE] flex items-center gap-2"><DollarSign size={20}/> {transacaoEditando ? 'Editar Lançamento' : 'Novo Lançamento'}</h2>
              <button onClick={() => setIsModalOpen(false)} className="text-gray-400 hover:text-white"><X size={24} /></button>
            </div>
            
            <form onSubmit={handleSalvarTransacao} className="flex flex-col overflow-hidden">
              <div className="p-6 space-y-5 overflow-y-auto custom-scrollbar flex-1">
                ...
                <div>
                  ...
                  <input type="text" name="descricao" defaultValue={transacaoEditando?.descricao} required placeholder={tipoTransacao === 'entrada' ? "Ex: Alongamento F1" : "Ex: Compra de Esmaltes"} className="w-full bg-[#2D0A12]/50 border border-[#DCAE96]/30 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-[#F8D1BE] transition-colors"/>
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[#E8D3C8] mb-1.5 uppercase tracking-wider font-bold text-[10px]">Valor (R$) *</label>
                    <input type="number" step="0.01" min="0" name="valor" defaultValue={transacaoEditando?.valor} required placeholder="0.00" className="w-full bg-[#2D0A12]/50 border border-[#DCAE96]/30 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-[#F8D1BE] transition-colors font-mono"/>
                  </div>
                  <div>
                    <label className="block text-[#E8D3C8] mb-1.5 uppercase tracking-wider font-bold text-[10px]">Data *</label>
                    <input type="date" name="data" required defaultValue={transacaoEditando ? new Date(transacaoEditando.data_pagamento).toISOString().split('T')[0] : new Date().toISOString().split('T')[0]} className="w-full bg-[#2D0A12]/50 border border-[#DCAE96]/30 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-[#F8D1BE] transition-colors color-scheme-dark"/>
                  </div>
                </div>

                <div>
                  <label className="block text-[#E8D3C8] mb-1.5 uppercase tracking-wider font-bold text-[10px]">Categoria</label>
                  <select name="categoria" defaultValue={transacaoEditando?.categoria} className="w-full bg-[#2D0A12]/50 border border-[#DCAE96]/30 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-[#F8D1BE] appearance-none transition-colors">
              ...
              <div className="px-6 py-4 bg-[#2D0A12] border-t border-[#DCAE96]/20 shrink-0">
                <button type="submit" disabled={isSaving} className={`w-full text-[#120308] px-8 py-3.5 rounded-xl font-bold hover:scale-[1.02] transition-transform disabled:opacity-50 flex justify-center items-center gap-2 shadow-lg ${tipoTransacao === 'entrada' ? 'bg-emerald-400 hover:bg-emerald-300' : 'bg-red-400 hover:bg-red-300'}`}>
                  {isSaving ? <Loader2 className="animate-spin mx-auto" size={20} /> : transacaoEditando ? 'Salvar Alterações' : 'Salvar Registro Financeiro'}
                </button>
              </div>
            </form>
          </div>
