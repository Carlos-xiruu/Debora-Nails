        ...
        <form onSubmit={handleSalvarConfiguracoes} className="space-y-6">
          ...
          <div className="bg-[#120308]/80 border border-[#DCAE96]/20 p-6 rounded-3xl shadow-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-sm text-[#E8D3C8] mb-1">Tipo de Chave</label>
                <select value={tipoPix} onChange={(e) => setTipoPix(e.target.value)} className="w-full bg-[#2D0A12]/50 border border-[#DCAE96]/30 rounded-xl px-4 py-3 text-sm text-white focus:outline-none appearance-none">
              ...
              <div className="col-span-2">
                <label className="block text-sm text-[#E8D3C8] mb-1">Sua Chave PIX Oficial</label>
                <input type="text" value={chavePix} onChange={(e) => setChavePix(e.target.value)} placeholder="Cole sua chave aqui..." className="w-full bg-[#2D0A12]/50 border border-[#DCAE96]/30 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-[#F8D1BE]"/>
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#120308]/80 border border-[#DCAE96]/20 p-6 rounded-3xl shadow-lg">
            ...
            <div>
              <label className="block text-sm text-[#E8D3C8] mb-1">Texto de Confirmação (WhatsApp)</label>
              <textarea value={msgConfirmacao} onChange={(e) => setMsgConfirmacao(e.target.value)} rows={4} className="w-full bg-[#2D0A12]/50 border border-[#DCAE96]/30 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-[#F8D1BE] resize-none"></textarea>
          ...
          <button type="submit" disabled={isSaving} className="w-full md:w-auto bg-gradient-to-r from-[#F8D1BE] to-[#C7977D] text-[#120308] px-10 py-4 rounded-xl font-bold flex justify-center items-center gap-2 hover:scale-[1.02] transition-transform">
