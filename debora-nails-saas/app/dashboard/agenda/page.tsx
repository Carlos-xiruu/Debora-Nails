...
export default function DashboardPage() {
  ...
  // 🛡️ MOTOR DE CANCELAMENTO INTELIGENTE
  const processarCancelamento = async (tipo: 'falta' | 'reembolso_total' | 'reembolso_parcial') => {
    ...
    try {
      ...
      // 1. Marca como Cancelado na Agenda
      ...
      if (tipo === 'falta' && clienteId) {
        ...
        await supabase.from('clientes').update({ faltas: faltasAtuais + 1 }).eq('id', clienteId);
      } 
      ...
      else if (tipo === 'reembolso_parcial' && valorSinal > 0) {
        await supabase.from('transacoes').insert([{
          descricao: `Reembolso 50% (-48h) - ${agendamentoCancelar.clientes?.nome}`,
          tipo: 'saida',
          valor: valorSinal / 2,
          categoria: 'Reembolso',
          data_pagamento: new Date().toISOString()
                ...
                <div className="p-6">
                  ...
                  <div className="space-y-3">
                    <button onClick={() => processarCancelamento('falta')} disabled={isProcessando} className="w-full bg-[#2D0A12] border border-red-500/30 p-4 rounded-xl text-left hover:bg-red-900/20 transition-all group disabled:opacity-50">
                      ...
                      <span className="text-xs text-gray-400 block leading-relaxed">Penaliza a cliente com +1 Falta no CRM. Retém 100% do sinal no seu caixa. A vaga volta para a agenda online.</span>
                    </button>
                    ...
                    <button onClick={() => processarCancelamento('reembolso_total')} disabled={isProcessando} className="w-full bg-[#2D0A12] border border-emerald-500/30 p-4 rounded-xl text-left hover:bg-emerald-900/20 transition-all group disabled:opacity-50">
                      ...
                      <span className="text-xs text-gray-400 block leading-relaxed">Não penaliza a cliente. Registra uma saída de 100% do sinal no seu financeiro como "Reembolso". Vaga liberada.</span>
                    </button>
                    ...
                    <button onClick={() => processarCancelamento('reembolso_parcial')} disabled={isProcessando} className="w-full bg-[#2D0A12] border border-orange-500/30 p-4 rounded-xl text-left hover:bg-orange-900/20 transition-all group disabled:opacity-50">
                      ...
                      <span className="text-xs text-gray-400 block leading-relaxed">Registra uma saída de 50% do sinal no seu financeiro. Os outros 50% ficam no caixa. Vaga liberada.</span>
                    </button>
                  </div>
                </div>
              </div>
