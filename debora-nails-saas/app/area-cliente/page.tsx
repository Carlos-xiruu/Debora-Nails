...
import { CalendarDays, Crown, ArrowLeft, LogOut, Loader2, Sparkles, Clock, History, Award, Gift, HeartHandshake, CheckCircle2 } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-[#0a0204] text-white font-sans selection:bg-[#C7977D] selection:text-[#120308] pb-20">
      ...
      <main className="max-w-5xl mx-auto px-6 pt-12 relative z-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
       ...
       {/* STATUS DE FIDELIDADE (GAMIFICAÇÃO) E VITRINE */}
        ...
        <h3 className="font-serif text-2xl text-white mb-6 flex items-center gap-2"><History className="text-[#C7977D]"/> Seu Histórico</h3>
        ...
        {agendamentos.length === 0 ? (
          <div className="bg-[#120308]/60 border border-[#3a2522] border-dashed rounded-[24px] p-12 text-center flex flex-col items-center">
            ...
            <p className="text-gray-400 text-lg">Você ainda não possui histórico de serviços.</p>
              ...
              return (
                <div key={ag.id} className="bg-[#180A0D] border border-[#3a2522] rounded-2xl p-5 hover:border-[#DCAE96]/30 transition-colors flex flex-col">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <span className={`text-[10px] uppercase font-bold tracking-widest px-2.5 py-1 rounded border ${statusCor} mb-2 inline-block`}>
                        {ag.tipo.replace('_', ' ')}
                      </span>
                      <h4 className="font-serif text-lg text-white leading-tight">{ag.servicos?.nome || 'Serviço Personalizado'}</h4>
                    </div>
                  </div>
                  
                  <div className="mt-auto border-t border-[#3a2522] pt-4 flex justify-between items-center text-sm">
                    <div className="flex items-center gap-1.5 text-gray-400">
                      <Clock size={14} className="text-[#C7977D]"/> {dataFormatada} às {horaFormatada}
                    </div>
                    {ag.servicos?.preco && (
                      <span className="font-bold text-[#DCAE96]">R$ {ag.servicos.preco.toFixed(2).replace('.', ',')}</span>
                    )}
                  </div>
                </div>
              );
